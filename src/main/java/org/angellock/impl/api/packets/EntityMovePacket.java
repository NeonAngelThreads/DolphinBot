...
public class EntityMovePacket extends AbstractEventProcessor<ClientboundMoveEntityPosPacket> {
    public EntityMovePacket() {
        this.preAction = (packet) -> {
            Position position = new Position(packet.getMoveX(), packet.getMoveY(), packet.getMoveZ());
            EntityMovedEvent entityMovedEvent = new EntityMovedEvent(packet.getEntityId(), position);
            dispatch(entityMovedEvent);
        };
    }
